...
var RegisterChatArdsClient = function () {
  ...
  try {
    var ardsReqServerUrl = util.format(
      "http://%s/DVP/API/%s/ARDS/requestserver",
...
var RemoveArdsRequest = function (
  tenant,
  company,
  sessionId,
  reason,
  ...
  try {
    var ardsReqServerUrl = util.format(
      "http://%s/DVP/API/%s/ARDS/request/%s/%s",
      config.Services.ardsliteservice,
      config.Services.ardsliteversion,
      sessionId,
      reason
    ...
    httpDelete(companyInfo, ardsReqServerUrl, function (err, res1, result) {
      if (err) {
        logger.error(
          "DVP-IPMessagingService.RemoveArdsRequest:: Error::" + err
        );
...
var UpdateResource = function (
  ...
  try {
    if (sessionId && company && tenant && resourceId) {
      logger.debug(
        "[DVP-IPMessagingService.SendResourceStatus] -  Creating PUT Message"
      );
      ...
      if (ardsIp && ardsPort && ardsVersion) {
        var httpUrl = util.format(
          "http://%s/DVP/API/%s/ARDS/resource/%s/concurrencyslot/session/%s?direction=%s",
          ardsIp,
          ardsVersion,
          resourceId,
          sessionId,
        ...
        httpPut(companyInfo, httpUrl, jsonObj, function (err, res1, result) {
          if (err) {
            logger.error(
              "[DVP-IPMessagingService.SendResourceStatus] - Set Resource Status Fail - Error : [%s]",
              err
            );
          } else {
            if (res1.statusCode === 200) {
              logger.debug(
                "[DVP-IPMessagingService.SendResourceStatus] - Set Resource Status Success : %s",
                result
              );
            } else {
              logger.error(
                "[DVP-IPMessagingService.SendResourceStatus] - Set Resource Status Fail - Response : [%s]",
                JSON.stringify(res1)
              );
            }
          }
        ...
        logger.error(
          "[DVP-IPMessagingService.SendResourceStatus] - ARDS Endpoints not defined",
          new Error("ARDS Endpoints not defined")
        );
      }
    }
  } catch (ex) {
    logger.error(
      "[DVP-IPMessagingService.SendResourceStatus] - Exception Occurred",
      ex
    );
  }
};
...
var GetOngoingSessions = function (tenant, company, resourceId, callback) {
  ...
  try {
    var ardsResourceUrl = util.format(
      "http://%s/DVP/API/%s/ARDS/resource/%s",
      config.Services.ardsliteservice,
      config.Services.ardsliteversion,
      resourceId
    );
    ...
    httpGet(companyInfo, ardsResourceUrl, function (err, res1, result) {
      if (err) {
        logger.error(
          "DVP-IPMessagingService.GetOngoingSessions:: Error::" + err
        );
    ...
    logger.error(
      "DVP-IPMessagingService.GetOngoingSessions:: Exception::" + ex
    );
