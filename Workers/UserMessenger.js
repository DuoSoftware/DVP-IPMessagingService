...
var UpdateRead = function (uuid) {
  PersonalMessage.findOneAndUpdate(
    { uuid: uuid },
    { status: "seen" },
...
function set_chat_session(tenant, company, session_id, reason, agent_data) {
  try {
    var jsonString;
    ards.RemoveArdsRequest(tenant, company, session_id, "NONE", function (
      err,
      res
    ) {
      jsonString = messageFormatter.FormatMessage(
        err,
        "accept chat - RemoveArdsRequest",
        true,
        res
      );
    ...
    redisClient.set(key, JSON.stringify(agent_data), function (err, obj) {
      if (err) {
        jsonString = messageFormatter.FormatMessage(
          undefined,
          "set_chat_session - fail to set assigned agent",
          false,
          undefined
        );
        ...
        jsonString = messageFormatter.FormatMessage(
          undefined,
          "set_chat_session - set assigned agent",
          true,
          obj
        );
    ...
    redisClient.hget(bot_usr_redis_id, data.sessionId, function (err, obj) {
      if (obj) {
        ...
        if (session_data.communication_type === "http") {
          var service_url = util.format(
            "http://%s/DVP/API/%s/IPMessengerAPI/Massage/%s",
            config.Services.ipmessagingapiurl,
            config.Services.ipmessagingapiversion,
            data.jti
          );
          if (validator.isIP(config.Services.ipmessagingapiurl)) {
            service_url = util.format(
              "http://%s:%s/DVP/API/%s/IPMessengerAPI/Massage/%s",
              config.Services.ipmessagingapiurl,
              config.Services.ipmessagingapiport,
              config.Services.ipmessagingapiversion,
              data.jti
            );
          }
          ...
          Common.http_post(
            ...
            util.format(
              "%d:%d",
              session_data.client_data.tenant,
              session_data.client_data.company
            )
          );
...
io.sockets
  ...
  .on("authenticated", function (socket) {
    ...
    redisClient.set(
      util.format("%s:messaging:time", socket.decoded_token.iss),
      Date.now().toString(),
      redis.print
    );
    ...
    var statusGroup = util.format(
      "%d:%d:messaging:status",
      socket.decoded_token.tenant,
      socket.decoded_token.company
    );
    socket.join(statusGroup);

    var fromRedisKey = util.format(
      "%s:messaging:status",
      socket.decoded_token.iss
    );
    var onlineUsers = util.format(
      "%d:%d:users:online",
      socket.decoded_token.tenant,
      socket.decoded_token.company
    );
    ...
    redisClient.get(fromRedisKey, function (errGet, resGet) {
      if (errGet) {
        ...
        if (!resGet) {
          ...
          ards.GetOngoingSessions(
            socket.decoded_token.tenant,
            socket.decoded_token.company,
            socket.decoded_token.context.resourceid,
            function (err, ongoinSessions) {
              if (ongoinSessions && ongoinSessions.length > 0) {
                ongoinSessions.forEach(function (session) {
                  var onlineclients = util.format(
                    "%d:%d:client:online:%s",
                    socket.decoded_token.tenant,
                    socket.decoded_token.company,
                    session
    ...
    socket.on("status", function (data) {
      if (data && data.presence) {
        ...
        var statusKey = util.format(
          "%s:messaging:status",
          socket.decoded_token.iss
        );

        if (data.presence_type === "call") {
          statusKey = util.format(
            "%s:%s:status",
            socket.decoded_token.iss,
            data.presence_type
          );
        ...
        if (data.presence_type == "call") {
          ...
          var onlineUsersStatus = util.format(
            "%d:%d:%susers:online",
            socket.decoded_token.tenant,
            socket.decoded_token.company,
            data.presence_type
            ...
            if (err) {
              ...
              if (user) {
                ...
                ards.UpdateResource(
                  ...
                  client_data.company,
                  data.to,
                  client_data.context.resourceid,
                  "Connected",
                  "",
                  "",
                  "inbound"
                ...
                var onlineClientsUsers = util.format(
                  "%d:%d:client:online:%s",
                  client_data.tenant,
                  client_data.company,
                  data.jti
                );
                var messageBufferKey = util.format(
                  "%d:%d:client:buffer:%s",
                  client_data.tenant,
                  client_data.company,
                  data.jti
                ...
                redisClient.set(onlineClientsUsers, jsonData, function (
                  clientUserSet
                ) {
                  redisClient.lrange(messageBufferKey, 0, -1, function (
                    err,
                    replies
                  ) {
                    ...
                    replies.forEach(function (reply, index) {
                      console.log(
                        "Reply " + index + ": " + JSON.parse(reply).title
                      );
    ...
    socket.on("reject", function (data) {
      if (data && data.to) {
        ...
        io_emit_message("agent_rejected", "to", data, {
          agent_rejected: client_data,
        });
      }
    });
    ...
    socket.on("sessionend", function (data) {
      if (data && data.to) {
        ...
        io_emit_message("sessionend", "to", data, {
    ...
    socket.on("typingstoped", function (data) {
      if (data && data.to) {
        ...
        io_emit_message("typingstoped", "to", data, {
          typingstoped: socket.decoded_token,
        });
      }
    });
              ...
              PersonalMessage.find(query)
                ...
                .exec(function (err, newmessages) {
                  if (data) {
                    ...
                    io.to(socket.decoded_token.iss).emit(
                      "newmessages",
                      newmessages
                    );
                  } else {
    ...
    socket.on("disconnect", function (reason) {
      ...
      io.sockets.adapter.clients([socket.decoded_token.iss], function (
        err,
        clients
      ) {
        if (err) {
          ...
          redisClient.set(
            util.format("%s:messaging:lastseen", socket.decoded_token.iss),
            Date.now(),
            redis.print
          );
        } else {
